export const StarsReview: React.FC<{ rating: number, size: number }> = (props) => {
  ...
  // if rating is 3 it will add 3 fullstars 0 half star and 2 empty star
  if (rating !== undefined && rating > 0 && rating <= 5) { // defines rating  from  0 to 5
      for (let i = 0; i <= 4; i++) {
          if (rating - 1 >= 0) {
  ...
  return ( // one fullStar  // one halfStar  // one emptyStar
      <div>
